  ...
  actions: {
    ...
    loadMessages() {
      try {
        ...
        this.messages = parsed.filter((message: unknown) =>
          this.isValidChatMessage(message),
        );
      } catch (e) {
        this.messages = [];
    ...
    executeCommand(command: ModelCommand) {
      ...
      switch (command.type) {
        case "add_place": {
          const pos =
            (command.params.position as { x: number; y: number }) ||
        ...
        case "add_arc": {
          ...
          if (!net) break;
          const { sourceId, targetId } = resolveArcEndpoints(
            net,
            command.params,
          );
          if (sourceId && targetId) {
            ...
            if (!arc) {
              chatLogger.warn(
                `add_arc skipped: invalid arc ${sourceId} → ${targetId}`,
              );
            }
          } else {
            chatLogger.warn(
        ...
        case "remove_element": {
          ...
          const elementId = net
            ? resolveElementId(
                net,
                command.params.element_id ?? command.params.elementId,
              )
            : null;
          if (elementId) {
